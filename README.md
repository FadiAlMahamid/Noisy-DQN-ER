...
| **Factorised Gaussian Noise** | Uses O(p+q) noise vectors instead of O(p*q) independent noise, transformed by f(x) = sign(x) * sqrt(\|x\|) for heavier tails |
...
NoisyLinear:        y = (mu_w + sigma_w * eps_w) x + (mu_b + sigma_b * eps_b)

where:
  mu_w, sigma_w, mu_b, sigma_b  — learnable parameters (optimized by gradient descent)
  eps_w, eps_b                   — factorised noise (resampled each learning step)
```
