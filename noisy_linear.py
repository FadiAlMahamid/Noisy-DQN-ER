...
class NoisyLinear(nn.Module):
    ...
    which is more parameter-efficient than independent noise:
        w = mu_w + sigma_w * (eps_out outer eps_in)
        b = mu_b + sigma_b * eps_out

    where eps_in and eps_out are factorised noise vectors transformed by f(x) = sign(x) * sqrt(|x|).
    """
    ...
    def reset_parameters(self):
        ...
        mu is initialized uniformly in [-1/sqrt(p), 1/sqrt(p)] where p = in_features,
        following the factorised noise initialization from the paper.
        sigma is initialized to sigma_init / sqrt(dim) so that the initial noise
    ...
    def _scale_noise(self, size):
        """
        Generates factorised noise using the f(x) = sign(x) * sqrt(|x|) transform.
    ...
    def forward(self, x):
        """
        Forward pass with noisy weights: w = mu + sigma * epsilon.
